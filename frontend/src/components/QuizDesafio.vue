<template>
  <div class="quiz-container">
    <div class="quiz-card">
      <h2>Desafío de Conocimientos</h2>
      ...
      <template v-else-if="!quizFinished">
        ...
        <template v-else>
          <p>No hay preguntas disponibles para el desafío.</p>
          <button class="result-btn" @click="goHome">Volver</button>
        </template>
      </template>

      <!-- Scoreboard al terminar las preguntas -->
      <template v-else>
        <div class="scoreboard">
          ...
          <p><strong>Incorrectas:</strong> {{ totalQuestions - (finalResult?.correct_answers ?? 0) }}</p>
  ...
  setup() {
    ...
    async function createChallenge() {
      ...
      if (!resp.ok) {
        ...
        errorMessage.value = `Error creando desafío: ${resp.status} ${txt}`;
    ...
    // Ciclo de vida
    onMounted(async () => {
      if (!(await createChallenge())) return;
    ...
    return {
      loading, errorMessage, currentQuestion,
